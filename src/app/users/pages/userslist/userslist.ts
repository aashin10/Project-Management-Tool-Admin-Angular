...
export class Userslist implements OnInit, OnDestroy {
  ...
  getChangedFields(): string[] {
    ...
    if (this.editUser.jiraId !== this.originalEditUser.jiraId) {
      changes.push(
        `Jira ID: "${this.originalEditUser.jiraId || '(empty)'}" → "${
          this.editUser.jiraId || '(empty)'
        }"`
      );
    }
  ...
  updateSelectionsForFilteredUsers() {
    ...
    const filteredUserKeys = new Set(
      this.filteredUsers.map((user) => user.user + '|' + user.email)
    );
    ...
    this.usersApi
      ...
      .subscribe({
        ...
        error: (error) => {
          ...
          this.toastr.error(error.message || 'Failed to export users', 'Export Failed', {
            timeOut: 5000,
            progressBar: true,
            closeButton: true,
          });
  ...
  fetchUsers() {
    ...
    // Call paginated API
    this.retrySubscription = this.usersApi
      .getPaginatedUsers({
      ...
      .subscribe({
        next: (response) => {
          ...
          const totalPages = Math.ceil(
            this.paginationState.totalCount / this.paginationState.pageSize
          );
  ...
  onSuspendUsers() {
    ...
    this.selectedUsers.forEach((selectedUser) => {
      const user = this.users.find(
        (u) => u.user === selectedUser.user.name && u.email === selectedUser.user.email
      );
      if (user) {
  ...
  confirmDelete() {
    ...
    if (this.pendingDeleteAction === 'bulk') {
      ...
      userIdsToDelete = this.selectedUsers
        .map((selectedUser) => {
      ...
      const user = this.users.find(
        (u) => u.user === this.userToDelete.user.name && u.email === this.userToDelete.user.email
      );
      if (user) {
    ...
    this.usersApi.deleteUsers(userIdsToDelete).subscribe({
      next: (response) => {
        ...
        this.notificationService.addNotification('success', successMessage, 'User Deleted');
        ...
        this.usersApi.bulkImportUsers(users, 1).subscribe({
          next: (response) => {
            ...
            setTimeout(() => {
              ...
              if (data.errorCount > 0) {
                this.toastr.error(`Errors: ${data.errorCount}`, 'Import Errors', {
                  timeOut: 8000,
                  progressBar: true,
                  closeButton: true,
                });
          ...
          error: (error) => {
            ...
            setTimeout(() => {
              ...
              this.notificationService.addNotification('error', errorMessage, 'Import Failed');
            }, 0);
          },
        });
      } catch (error) {
